...
#
# Writes: dist/syphon-filter-3-recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix syphon-filter-3-recomp \
  --exe-name Syphon_Filter_3_Recompiled \
  --display-name "Syphon Filter 3 Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Syphon Filter 3 disc" \
